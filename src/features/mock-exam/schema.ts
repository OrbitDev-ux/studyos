import { z } from "zod";
import { AI_MAX_QUESTIONS } from "@/features/ai/generation-limits";

export const mockExamGenerationFormSchema = z
  .object({
    ...
    count: z.coerce
      ...
      .int()
      .min(5, "5문항 이상 입력해주세요")
      .max(AI_MAX_QUESTIONS, `최대 ${AI_MAX_QUESTIONS}문항까지 가능합니다`),
  ...
  // 객관식 + 서술형을 합쳐도 전체 문항이 상한을 넘지 않도록 한다.
  .refine((d) => d.count + d.essayCount <= AI_MAX_QUESTIONS, {
    path: ["count"],
    message: `객관식과 서술형을 합쳐 최대 ${AI_MAX_QUESTIONS}문항까지 가능합니다`,
  });
