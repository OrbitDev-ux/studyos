...
const QUICK_COMMANDS = ["npm run dev", "npm run build", "python3 main.py", "node index.js"];
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-1.5">
        {QUICK_COMMANDS.map((cmd) => (
          <Button key={cmd} type="button" size="sm" variant="outline" className="font-mono" onClick={() => run(cmd)} disabled={pending}>
            {cmd}
          </Button>
        ))}
      </div>
      ...
      <div className="flex gap-2">
        <Input
          ...
          className="font-mono"
        />
        <Button type="button" onClick={() => command.trim() && run(command)} disabled={pending} className="gap-1.5">
              ...
              <CardContent className="flex flex-col gap-2 py-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <p className="truncate font-mono text-sm font-medium">{proc.command}</p>
                    <p className="text-muted-foreground text-xs">{proc.id.split(":")[1]}</p>
                  </div>
                  <div className="flex shrink-0 items-center gap-1.5">
                    <StatusBadge status={proc.status} t={t} />
                    {proc.status === "RUNNING" ? (
                      <Button type="button" size="icon-sm" variant="ghost" onClick={() => stopWorkspaceProcess(proc.id).then(refresh)}>
                        <Square className="size-3.5" />
                      </Button>
                    ) : (
                      <Button type="button" size="icon-sm" variant="ghost" onClick={() => restartWorkspaceProcess(proc.id).then(refresh)}>
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="ghost"
                      onClick={() => setExpandedId((id) => (id === proc.id ? null : proc.id))}
                    >
                      {t.runLogs}
                    </Button>
                  </div>
...
function StatusBadge({ status, t }: { status: ProcessRecord["status"]; t: { statusRunning: string; statusStopped: string; runStatusFailed: string } }) {
  const label = status === "RUNNING" ? t.statusRunning : status === "FAILED" ? t.runStatusFailed : t.statusStopped;
