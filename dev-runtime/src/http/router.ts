...
function readJsonBody(req: IncomingMessage): Promise<unknown> {
  return new Promise((resolve, reject) => {
    const chunks: Buffer[] = [];
    let size = 0;
    const MAX_BODY_BYTES = 2_000_000; // 2MB — file writes go through a dedicated larger-limit route
...
/** Never leaks internal error details to the caller (§35) — only a stable
 * machine code + safe message. Full detail goes to server logs by the caller. */
  ...
  async handle(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
    ...
    for (const route of this.routes) {
      ...
      try {
        ...
        respondError(res, 500, "internal_error", "Something went wrong.");
      }
      return true;
    }
    return false;
